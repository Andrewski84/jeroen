...
// Load useful links for dropdown
$linksItems = [];
if (defined('LINKS_FILE') && file_exists(LINKS_FILE)) {
    $ld = json_decode(file_get_contents(LINKS_FILE), true) ?: [];
    $linksItems = $ld['items'] ?? [];
}
?>
<header id="header" class="<?php echo $headerClass; ?> top-0 left-0 w-full z-50">
    <nav class="container mx-auto px-6 flex justify-between items-center h-24">
        ...
        <div class="hidden md:flex items-center space-x-10 absolute left-1/2 -translate-x-1/2">
            ...
            <div class="relative header-dropdown">
                <a href="links.php" class="nav-link<?php echo $page === 'links' ? ' active' : ''; ?> flex items-center gap-2" aria-haspopup="true" aria-expanded="false">
                    <span>Nuttige links</span>
                    <svg class="w-4 h-4" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 10.94l3.71-3.71a.75.75 0 111.06 1.06l-4.24 4.24a.75.75 0 01-1.06 0L5.21 8.29a.75.75 0 01.02-1.08z" clip-rule="evenodd"/></svg>
                </a>
                <?php if (!empty($linksItems)): ?>
                <div class="dropdown-panel">
                    <?php foreach ($linksItems as $it): $url = trim($it['url'] ?? ''); $label = trim($it['label'] ?? ''); if ($label === '' || $url === '') continue; ?>
                        <a href="<?php echo htmlspecialchars(safeUrl($url)); ?>" target="_blank" class="dropdown-item"><?php echo htmlspecialchars($label); ?></a>
                    <?php endforeach; ?>
                </div>
                <?php endif; ?>
            </div>
        ...
        <nav class="p-4 flex flex-col gap-2">
            ...
            <div class="w-full">
                <div class="flex items-center gap-2">
                    <a href="practice.php" class="mobile-nav-link flex-1 block text-lg px-4 py-3 rounded-lg hover:bg-slate-50">Praktijkinfo</a>
                    <button id="mobile-practice-toggle" class="p-3 rounded-lg hover:bg-slate-50" aria-controls="mobile-practice-menu" aria-expanded="false">
            ...
            <div class="w-full">
                <div class="flex items-center gap-2">
                    <a href="links.php" class="mobile-nav-link flex-1 block text-lg px-4 py-3 rounded-lg hover:bg-slate-50">Nuttige links</a>
                    <button id="mobile-links-toggle" class="p-3 rounded-lg hover:bg-slate-50" aria-controls="mobile-links-menu" aria-expanded="false">
                        <svg class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 10.94l3.71-3.71a.75.75 0 111.06 1.06l-4.24 4.24a.75.75 0 01-1.06 0L5.21 8.29a.75.75 0 01.02-1.08z" clip-rule="evenodd"/></svg>
                    </button>
                </div>
                <?php if (!empty($linksItems)): ?>
                <div id="mobile-links-menu" class="hidden mt-2 grid gap-2 px-2">
                    <?php foreach ($linksItems as $it): $url = trim($it['url'] ?? ''); $label = trim($it['label'] ?? ''); if ($label === '' || $url === '') continue; ?>
                        <a href="<?php echo htmlspecialchars(safeUrl($url)); ?>" target="_blank" class="mobile-nav-link block text-base py-2 border border-[var(--border)] rounded-full text-center" style="background: var(--surface);">
            ...
            <?php endif; ?>
            <?php if (!empty($phoneNumber)): ?>
            <a href="tel:<?php echo htmlspecialchars($phoneNumber); ?>" class="mobile-nav-link block text-lg px-4 py-3 rounded-lg hover:bg-slate-50">Tel: <?php echo htmlspecialchars($phoneNumber); ?></a>
            <?php endif; ?>
            <?php if (!empty($_SESSION['loggedin'])): ?>
